...
class TurtleCoind {
  /**
   * Initializes a new TurtleCoind object
   * @constructor
  ...
  _post (method, params) {
    ...
    return this._rawPost('json_rpc', body).then((response) => {
   ...
   * @property {TurtleCoind.TransactionSummary[]} transactions
   ...
   * @returns {Promise<TurtleCoind.BlockSummary>} resolves with block summary or rejects with error
   */
  block (hash) {
   ...
   * @async
   * @param {string} hash - the hash of the block to retrieve
   * @returns {Promise<TurtleCoind.BlockHeader>} resolves with block header or rejects with error
   */
  ...
  blockHeaderByHeight (height) {
    ...
    this._post('getblockheaderbyheight', { height })
   ...
   * @async
   * @param {number} height - the height of the block to retrieve
   * @returns {Promise<TurtleCoind.BlockShortHeader[]>} resolves with block short headers or rejects with error
   */
   ...
   * @property {TurtleCoind.TransactionDetailInputCoinbaseInput} input - the input
   */
   ...
   * @property {TurtleCoind.TransactionDetailInputKeyInput} input - the input
   * @property {number} mixin - the ring size
   * @property {TurtleCoind.TransactionDetailInputKeyOutput} output - the related output information
   */
   ...
   * @property {TurtleCoind.TransactionDetailInputCoinbase|TurtleCoind.TransactionDetailInputKey} data - the input data
   ...
   * @property {TurtleCoind.TransactionOutputTargetData} data - output data structure
   ...
   * @property {TurtleCoind.TransactionOutputTarget} target - the output target
   */
   ...
   * @property {TurtleCoind.TransactionOutput} output - The transaction output
   */
   ...
   * @property {TurtleCoind.TransactionExtraDetail} extra - the transaction extra
   ...
   * @property {TurtleCoind.TransactionDetailInput[]} inputs - the inputs of the transaction
   * @property {number} mixin - the number of transaction mixins
   * @property {TurtleCoind.TransactionDetailOutput[]} outputs - the outputs of the transaction
   ...
   * @property {number} timestamp - the block timestamp
   * @property {number} totalFeeAmount - the total amount of fees in the block
   * @property {TurtleCoind.TransactionDetail[]} transactions
   * @property {number} transactionsCumulativeSize - the total size of the transactions in the block
   */
   ...
   * @property {TurtleCoind.BlockDetails} blocks - the blocks
   ...
   * @returns {Promise<TurtleCoind.BlocksDetailedResponse>} resolves with blocks detail information or rejects with error
   */
  blocksDetailed (opts) {
    opts = opts || {}
   ...
   * @property {string} hash - the block hash
   * @property {TurtleCoind.TransactionPrefix[]} transactions - the transactions in the block
   */
   ...
   * @property {TurtleCoind.BlockLite[]} items - the block data
   ...
   * @param {number} [opts.timestamp=0] - the timestamp to start from
   * @param {string[]} [opts.blockHashes] - the block hashes
   * @returns {Promise<TurtleCoind.BlocksLiteResponse>} resolves with block information or rejects with error
   */
  blocksLite (opts) {
    opts = opts || {}
    if (!Array.isArray(opts.blockHashes)) throw new Error('must supply an array of block hashes')
   ...
   * @returns {Promise<TurtleCoind.BlockTemplateResponse>} resolves with block template response or rejects with error
   */
   ...
   * @async
   * @returns {Promise<TurtleCoind.NodeFee>} resolves with node fee information or rejects with error
   */
  fee () {
   ...
   * @returns {Promise<TurtleCoind.GlobalIndexesResponse[]>} resolves with global indexes information or rejects with error
   */
  globalIndexesForRange (startHeight, endHeight) {
    ...
    this._rawPost('get_global_indexes_for_range', { startHeight, endHeight })
      .then(response => {
   ...
   * @async
   * @returns {Promise<TurtleCoind.NodeHeight>} resolves with node height information or rejects with error
   */
  height () {
   ...
   * @async
   * @returns {Promise<TurtleCoind.NodeInfo>} resolves with node information or rejects with error
   */
  info () {
   ...
   * @async
   * @returns {Promise<TurtleCoind.NodePeers>} resolves with node peer information or rejects with error
   */
  peers () {
   ...
   * @property {string} hash - the transaction hash
   * @property {TurtleCoind.TransactionPrefix} prefix - the transaction prefix
   */

  /**
   * @memberof TurtleCoind
   * @typedef PoolChanges
   * @property {TurtleCoind.PoolChangesAdded} addedTxs - the recently added pool transactions
   ...
   * @returns {Promise<TurtleCoind.PoolChanges>} resolves with pool change information or rejects with error
   */
  poolChanges (tailBlockHash, knownTransactionHashes) {
    ...
    this._rawPost('get_pool_changes_lite', body)
      .then(response => {
        /* We need to clean up the response a bit */
        const tmp = []

        response.addedTxs.forEach((tx) => {
          tmp.push({
   ...
   * @property {number} amount - the amount of the output
   * @property {TurtleCoind.RandomOutput[]} outs - a list of random outputs
   */
   ...
   * @property {TurtleCoind.RandomOuts[]} outs - a list of random outs
   * @property {string} status - the response status
   */
   ...
   * @returns {Promise<TurtleCoind.RandomOutputsResponse>} resolves with random outputs information or rejects with error
   */
  randomOutputs (opts) {
    opts = opts || {}
    if (!Array.isArray(opts.amounts)) throw new Error('must supply an array of amounts')
    if (typeof opts.mixin === 'undefined') throw new Error('must supply a mixin value')

    opts.mixin = parseInt(opts.mixin)
    if (isNaN(opts.mixin)) throw new Error('must supply a valid mixin value')
   ...
   * @property {TurtleCoind.RawTransaction[]} transactions - a list of raw transactions
   * @property {number} tx_count - the number of transactions in the block
   */
   ...
   * @property {TurtleCoind.RawBlock[]} blocks - the raw blocks
   ...
   * @returns {Promise<TurtleCoind.RawBlocksResponse>} resolves with raw blocks information or rejects with error
   */
   ...
   * @returns {Promise<TurtleCoind.SendRawTransactionResponse>} resolves with send raw transaction information or rejects with error
   */
   ...
   * @returns {Promise<TurtleCoind.SubmitBlockResponse>} resolves with submit block response or rejects with error
   */
   ...
   * @memberof TurtleCoind
   * @typedef VOUTTarget
   * @property {TurtleCoind.VOUTTargetData} data - output target data
   ...
   * @typedef VOUT
   * @property {number} amount - the amount of the output
   * @property {TurtleCoind.VOUTTarget} target - the output target
   */
   ...
   * @property {TurtleCoind.VINCoinbase|TurtleCoind.VIN[]} vin - the transaction inputs
   * @property {TurtleCoind.VOUT[]} vout - the transaction outputs
   */
   ...
   * @property {string} status - the status of the request
   * @property {TurtleCoind.TransactionPrefix} tx - the transaction structured information
   * @property {TurtleCoind.TransactionMetadata} txDetails - the transaction meta information
   */
   ...
   * @returns {Promise<TurtleCoind.TransactionResponse>} resolves with transaction response or rejects with error
   */
  transaction (hash) {
    if (!hash) throw new Error('must specify hash')
   ...
   * @async
   * @returns {Promise<TurtleCoind.TransactionSummary[]>} resolves with the transaction summaries or rejects with error
   */
  transactionPool () {
   ...
   * @returns {Promise<TurtleCoind.TransactionsStatusResponse>} resolves with the transactions statuses or rejects with error
   */
  transactionsStatus (transactionHashes) {
    if (!Array.isArray(transactionHashes)) throw new Error('Must specify transaction hashes')

    this._rawPost('get_transactions_status', { transactionHashes })
      .then(response => {
   ...
   * @property {string} hash - the transaction hash
   * @property {TurtleCoind.TransactionDetailInputKeyInput} [inputs] - the transaction inputs
   * @property {TurtleCoind.WalletSyncTransactionOutput[]} outputs - the transaction outputs
   ...
   * @property {TurtleCoind.WalletSyncTransaction} [coinbaseTX] - the block coinbase transaction
   * @property {TurtleCoind.WalletSyncTransaction[]} transactions - the transactions in the block
   */
   ...
   * @property {TurtleCoind.WalletSyncDataBlock[]} items - block data array
   * @property {string} status - the reponse status message
   * @property {boolean} synced - whether the request is fully synched
   * @property {TurtleCoind.WalletSyncDataTopBlock} [topBlock] - the top block information
   */
   ...
   * @returns {Promise<TurtleCoind.WalletSyncDataResponse>} resolves with sync data response or rejects with error
   */
  walletSyncData (opts) {
    opts = opts || {}
