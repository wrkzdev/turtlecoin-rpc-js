...
Self.prototype.toAtomicUnits = function (amount) {
  ...
  if (parseInt(amount) === amount) return amount
...
Self.prototype.transactionByHash = function (hash) {
  ...
  return new Promise((resolve, reject) => {
    ...
    this._get(url).then((result) => {
      /* Convert amounts to human readable amounts */
      result.transactions.fee = this.fromAtomicUnits(result.transactions.fee)

      for (var i = 0; i < result.transactions.transfers.length; i++) {
        result.transactions.transfers[i].amount = this.fromAtomicUnits(result.transactions.transfers[i].amount)
      }
...
Self.prototype.transactionsByAddress = function (address, startHeight, endHeight) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var url = util.format('/transactions/%s', address)
...
Self.prototype.sendAdvanced = function (destinations, mixin, fee, sourceAddresses, paymentId, changeAddress, unlockTime) {
  ...
  return new Promise((resolve, reject) => {
    ...
    for (var i = 0; i < destinations.length; i++) {
      ...
      destinations[i].amount = this.toAtomicUnits(destinations.amount)
    }
...
Self.prototype._delete = function (path) {
  return new Promise((resolve, reject) => {
    ...
    /* Delete methods can take a while depending on what they are doing
       before responding to the request and as such we do not honor the
       timeouts for DELETE calls */
