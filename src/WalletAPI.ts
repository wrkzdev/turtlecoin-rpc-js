...
t     * Deletes a previous prepared transaction
    ...
    public async transactions (
        ...
        response.transactions = response.transactions.map((txn: WalletAPIInterfaces.ITransactionInfo) => {
            txn.fee = fromAtomicUnits(txn.fee, this.m_decimalDivisor);

            txn.transfers = txn.transfers.map((elem: WalletAPIInterfaces.ITransferDestination) => {
                elem.amount = fromAtomicUnits(elem.amount, this.m_decimalDivisor);
